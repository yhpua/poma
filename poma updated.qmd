...
```{r }
#| echo: TRUE
#' Example taken from https://search.r-project.org/CRAN/refmans/rms/html/impactPO.html
d   <- expand.grid(age=50, sex=c('female', 'male'))
toydf <- data.frame (
  age = rnorm(500, 50, 10),
  sex = sample(c('female', 'male'), 500, TRUE),
  y = sample(0:4, 500, TRUE))
mod.orm <- orm(y ~ age + sex, x = T, y = T, data = toydf )
poma(mod.orm)                        ## prompts user to use rms::impactPO
poma(mod.orm, newdata = d)           ## triggers rms::impactPO
```
...
poma(mod.orm, newdata = d, minfreq = 5) ## setting a lower `minfreq` value for rms::impactPO
```
 
## Long ordinal/continuous Response Variable
```{r }
#| echo: TRUE
d <- expand.grid(hp=c(90, 180), cyl = c(4, 6))
...
:::{.callout-tip .column-page collapse="true"}
## Updated poma.R
```{r }
...
#'d <- expand.grid(hp = c(90, 180), vs = c(0, 1))
#'mod.orm <- orm(cyl ~ vs + hp , x = TRUE, y = TRUE, data = mtcars)
#'poma(mod.orm, newdata = d, minfreq = 5)
#'
#'
